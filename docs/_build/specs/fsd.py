...
AREAS = [
    ...
    {"key": "FR-CTX", "name": "Context & ubiquitous language",
     "description": "Establishing one shared vocabulary — derived from the intent, scoped to bounded contexts, "
...
FLOWS = [
    {"name": "Flow A — Specification chain",
     ...
     "steps": [
         {"title": "Purpose", "desc": "A brief, a conversation, or rough notes.", "kind": "input"},
         {"title": "Decision gate", "desc": "Every fork resolved and written down before authoring starts.", "kind": "gate"},
         {"title": "Intent", "desc": "Problem, principles, personas, capabilities."},
...
REQUIREMENTS = [
    ...
    {"id": "FR-DOC-01", "area": "FR-DOC", "priority": "Must", "title": "Seven document types",
     "text": "The system shall produce seven specification document types — Intent, Context, PRD, FSD, User "
     ...
     "amendments": [
         {"date": "2026-09-02",
          "text": "The first of the seven was called the Vision when this was written and is called the Intent "
                  "from this set onwards. Only the name changed: the same generator writes it, it answers the "
                  "same question, and it still gates everything below it. A project that already holds a "
                  "Vision document keeps it and is read through the old name, so nothing on disk has to move "
                  "for the new name to work (NFR-OPS-07). The rename was made because the document states what "
                  "a piece of work is FOR, and 'vision' had begun to read as a slogan rather than a "
                  "specification."}],
     "tags": ["chain"]},
    ...
    {"id": "FR-DOC-10", "area": "FR-DOC", "priority": "Must", "title": "Source register",
     "text": "The intent generator shall accept any combination of narrative, source documents and web addresses "
     ...
     "amendments": [
         {"date": "2026-09-02",
          "text": "One intent generator serves two levels. The project's own Intent says what the project is "
                  "for; a feature's Intent says what that piece of work is for, and is written by the same "
                  "generator into the feature's own specification directory. Nothing is duplicated to make "
                  "this work: the generator is told which level it is writing at by where the paths resolve "
                  "to, not by a second implementation, and a second generator for the feature level would be "
                  "two places to change the same rule."}],
     "tags": ["intake", "provenance"]},

    # ---------------- FR-CTX ----------------
    {"id": "FR-CTX-01", "area": "FR-CTX", "priority": "Must", "title": "Context derived from the intent",
     "text": "The system shall generate a Context document only after a completed Intent exists, deriving its "
             "candidate vocabulary from the intent, its source register, and any supplied material — never from "
             "invention.",
     "amendments": [
         {"date": "2026-09-02",
          "text": "The Context is the one document a project holds once. Where work is organised as features, "
                  "every feature's chain reads the project's Context and no feature writes one of its own: a "
                  "term means the same thing in every piece of work, which is the whole point of a ubiquitous "
                  "language, and a per-feature glossary would be a second answer to what a word means."}],
     "tags": ["chain", "core"]},
    ...
    {"id": "FR-SKL-02", "area": "FR-SKL", "priority": "Must", "title": "Prerequisite enforcement",
     "text": "Each document-generating skill shall verify that its required upstream document exists and is "
             ...
             "is not.",
     "notes": "Intent needs nothing; Context needs Intent; PRD needs Context; FSD needs PRD; Stories and SDD each "
    ...
    {"id": "FR-SKL-05", "area": "FR-SKL", "priority": "Must", "title": "Resume from wherever the set stands",
     "text": "The system shall provide a resume skill that inspects the document set, determines the furthest "
             "completed step, and continues from there — starting from the beginning when no completed Intent "
             "exists.",
     "amendments": [
         {"date": "2026-09-02",
          "text": "Where a feature is open, the position resumed from is the feature's, not the project's. The "
                  "set inspected is the feature's own documents plus the shared ones no feature document "
                  "stands in for, so resuming inside a feature that has written its Intent and its PRD "
                  "continues at the FSD — not at the beginning, which is what reading the project's set alone "
                  "would have said."}],
    ...
    {"id": "FR-SKL-10", "area": "FR-SKL", "priority": "Must", "title": "Feature skill",
     "text": "The system shall provide a feature skill with exactly three operations — open a named feature, "
             "close the open one, and report which feature is open and what its closing audit would find — and "
             "shall take no argument naming which feature to act on, because the feature acted on is always the "
             "one the project derives.",
     "notes": "Three operations and no selector is the whole interface. A flag naming a feature would let an "
              "operator act on one the rest of the toolchain is not pointed at, which is the failure the "
    ...
    {"id": "FR-GEN-12", "area": "FR-GEN", "priority": "Must", "title": "Work is organised as features",
     "text": "The system shall allow a project to hold more than one piece of work by giving each its own "
             "numbered, named directory containing that work's specifications, plan and run state, while the "
             "project's own intent, shared vocabulary, worker configuration and design record stay above them "
             "and are held once. Which piece of work is current shall be derived from the directories that "
             "exist — the highest-numbered one — and shall never be configured, stored or passed as an "
             "argument. Each piece of work shall be its own universe of identifiers, proved on its own and "
             "never scanned against another's. A project that has opened none shall behave exactly as it did "
             "before this requirement existed, byte for byte.",
     "notes": "Derivation rather than configuration is the point: a stored answer to 'which feature am I in' "
              "is a second source of truth that goes stale the moment somebody makes a directory by hand. "
              "Numbering from 001 keeps the order on disk and the order in time the same thing.",
     "tags": ["features", "layout", "core"]},
    {"id": "FR-GEN-13", "area": "FR-GEN", "priority": "Must", "title": "One piece of work open at a time",
     "text": "The system shall refuse to open a second piece of work while one is open, naming the open one and "
             "what to do instead, and shall refuse to write into one that has been closed. Work that arrives "
             "while a piece of work is open — a correction, a small fix, a late requirement — shall be folded "
             "into the open one as an addendum rather than opening a second.",
     "notes": "Two open pieces of work would mean two answers to where a document goes, two plans a run could "
              "read, and two ledgers a status could be written into. The addendum door already exists for "
              "exactly this and needs nothing new.",
     "tags": ["features", "gate", "core"]},
    {"id": "FR-GEN-14", "area": "FR-GEN", "priority": "Must", "title": "Closing is audited",
     "text": "Closing a piece of work shall run an audit that reports every unit of its plan not passing, every "
             "retired identifier naming no successor, every question its documents left open, and everything it "
             "built that is not yet shipped. A close stating no reason shall require a clean audit and shall "
             "otherwise refuse, listing every finding. A close stating a reason shall succeed and record what "
             "the audit found alongside that reason. Either way the outcome — the date, the reason, and what "
             "was left — shall be written into that work's own first document, which shall remain "
             "byte-identical to a regeneration of it.",
     "notes": "Parking something unfinished is a legitimate decision; losing the list of what was unfinished is "
              "not. The two doors differ only in whether the operator has to say why.",
     "tags": ["features", "gate", "integrity"]},
]
