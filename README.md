...
Nine more skills operate on the set instead of extending it: `/zero:init` sets a
...
`/zero:feature` opens the next feature or closes the open one after an audit,
...
| `families` | `[]` | Writes that always move together, stated once: `[{"when": "drizzle/migrations/**", "also": ["drizzle/meta/_journal.json", "src/db/types.ts"]}]`. A unit whose declared writes touch `when` is read as writing every path in `also`, for both the stray check and the collision check — and a unit that WRITES `when` without declaring it gets the same for the stray check, because the units a family is for are the ones whose plan did not predict it. The keyed path itself is never covered that way. |
...
skills/         16 SKILL.md files, one per step, shipped as the `zero` plugin
