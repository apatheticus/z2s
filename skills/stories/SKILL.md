---
name: stories
description: Generates user stories and use cases. Invoke deliberately; never run unasked.
disable-model-invocation: true
argument-hint: [roles, journeys or acceptance rules to cover]
---
...
**Requires:** a completed Context.html and FSD.html. The command below checks and refuses by name.
...
`${CLAUDE_PLUGIN_ROOT}/z2s/stories.py`. Read that, not a copy of it, because a
...
```
.zero/state/briefs/stories.json
```

**3. Run the cycle.**

```
PYTHONPATH="${CLAUDE_PLUGIN_ROOT}" python3 -m z2s.author run stories --root .
```
...
- **exit 3** — a fork is open. Put the printed question to the operator through
  ...
  stories <fork> <choice> --why "<their reason>" --root .`, then run again.
