---
name: fsd
description: Generates the functional specification. Invoke deliberately; never run unasked.
disable-model-invocation: true
argument-hint: [requirement areas or capabilities to specify]
---
...
**Requires:** a completed Context.html and PRD.html. The command below checks and refuses by name.
...
`${CLAUDE_PLUGIN_ROOT}/z2s/fsd.py`. Read that, not a copy of it, because a
...
```
.zero/state/briefs/fsd.json
```

**3. Run the cycle.**

```
PYTHONPATH="${CLAUDE_PLUGIN_ROOT}" python3 -m z2s.author run fsd --root .
```
...
- **exit 3** — a fork is open. Put the printed question to the operator through
  ...
  fsd <fork> <choice> --why "<their reason>" --root .`, then run again.
