---
name: sdd
description: Generates the technical design. Invoke deliberately; never run unasked.
disable-model-invocation: true
argument-hint: [stack, components or decisions to record]
---
...
`${CLAUDE_PLUGIN_ROOT}/z2s/sdd.py`. Read that, not a copy of it, because a
...
```
.zero/state/briefs/sdd.json
```

**3. Run the cycle.**

```
PYTHONPATH="${CLAUDE_PLUGIN_ROOT}" python3 -m z2s.author run sdd --root .
```
...
- **exit 3** — a fork is open. Put the printed question to the operator through
  ...
  sdd <fork> <choice> --why "<their reason>" --root .`, then run again.
