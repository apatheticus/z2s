---
name: prd
description: Generates the product requirements. Invoke deliberately; never run unasked.
disable-model-invocation: true
argument-hint: [goals, journeys or measures to fold in]
---
...
**Requires:** a completed Vision.html and Context.html. The command below checks and refuses by name.
...
`${CLAUDE_PLUGIN_ROOT}/z2s/prd.py`. Read that, not a copy of it, because a
...
```
.zero/state/briefs/prd.json
```

**3. Run the cycle.**

```
PYTHONPATH="${CLAUDE_PLUGIN_ROOT}" python3 -m z2s.author run prd --root .
```
...
- **exit 3** — a fork is open. Put the printed question to the operator through
  ...
  prd <fork> <choice> --why "<their reason>" --root .`, then run again.
